...
const info = map.split('\n')
let validCount = 0;

// loop through line arrays
for (let i = 0; i < info.length; i++) {
    //create variables for min/maxCount,letter and password
    let minCount = info[i].split(" ")[0].split('-')[0]
    let maxCount = info[i].split(" ")[0].split('-')[1]
    let letter = info[i].split(" ")[1].replace(':', '');
    let password = info[i].split(":")[1];
    let letter_count = 0;


    //    //Test 1 validation loop through password to see if it //contains letter and increment if it does
    //      for(let passI=0; passI < password.length; passI++){
    //        if(password.charAt(passI) === letter){
    //          letter_count++;
    //        }
    //      }

    //      //if letters are between min and max Count. increment //validCount
    //     if(letter_count >= minCount && letter_count <= maxCount){
    //       validCount++;
    //     }

    //Test 2 validation check if min or max contain letter validation. if both are then it is not valid

    if (password.charAt(minCount) == letter && password.charAt(maxCount) !== letter ||
        password.charAt(maxCount) == letter && password.charAt(minCount) !== letter) {
        validCount++;
    }
}
